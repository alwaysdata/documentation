...
Nous fournissons un accès SMTP, IMAP et POP3, dont les adresses sont indiquées dans le menu **Emails > Adresses**. Si ce n'est depuis nos serveurs directement (par exemple, depuis une application HTTP), il est *nécessaire* de s'authentifier avec une adresse email pour envoyer des emails.

Nos serveurs emails sont classiques ; nous utilisons [Exim](https://www.exim.org/) et [Dovecot](https://www.dovecot.org/).

Vous pouvez [créer une adresse email](/fr/docs/emails/creer-une-adresse-email/) dans l'onglet **Emails > Adresses** de l'interface d'administration et [consulter vos emails](/fr/docs/emails/utiliser-une-adresse-email/) via un client email de votre choix ou notre [webmail](https://webmail.alwaysdata.com).
