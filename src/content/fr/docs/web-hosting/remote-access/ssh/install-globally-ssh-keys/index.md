...
![Interface d'administration : clés SSH globales](images/admin-interface_global-ssh-keys.png)

Votre clé SSH publique à copier sur ce formulaire est disponible dans un fichier du répertoire `$HOME/.ssh` de votre ordinateur (par exemple `$HOME/.ssh/id_ed25519.pub`). Si vous n'en avez pas vous pouvez la [générer](/fr/docs/hebergement-web/acces-distant/ssh/use-keys).
