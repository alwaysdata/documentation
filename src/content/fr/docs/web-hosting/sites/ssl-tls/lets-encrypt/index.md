...
Pour éviter les doublons, alwaysdata autorise la génération de certificats Let's Encrypt seulement pour des [certificats wildcard](/fr/docs/hebergement-web/sites/ssl-tls/certificats-lets-encrypt/#certificats-wildcard).
...
![Interface d'administration : certificats autogénérés](images/admin-panel_autogenerated-certificates.png)
...
Lorsqu'un domaine utilise nos [serveurs DNS](/fr/docs/caracteristiques-techniques/informations-de-connexion/), il est possible de générer un [certificat wildcard Let's Encrypt](https://en.wikipedia.org/wiki/Wildcard_certificate) - de la forme `*.example.org` - dans **Avancé > Certificats SSL > Ajouter un certificat SSL > Créer un certificat wildcard Let's Encrypt**. Ce certificat sera automatiquement renouvelé par le système.
