...
- Due to its specificities, it is not possible to set up an automatic response. This could otherwise cause waves of spam.
    However, it is possible to use it for specific recipients via [Sieve scripts](/en/docs/e-mails/use-sieve-scripts) by adding these recipient addresses to the vacation instructions:
