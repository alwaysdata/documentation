...
eleventyNavigation:
  key: Analyzing HTTP Processes
  parent: Websites
---

This function lets experienced developers observe the behavior of their applications and improve their performance.

The process analysis produces a file, available from the root of your account (called `analysis_[date]-[time].log` and that can be viewed in [SSH](/en/docs/web-hosting/remote-access/ssh) or [FTP](/en/docs/web-hosting/remote-access/ftp)) describing the operations performed by the process during a set period of time.
