---
title: Migrating a Website to alwaysdata
eleventyNavigation:
...
1.  Retrieve the files and the database from the current provider,
    - Copy the files to your alwaysdata account using [FTP](/en/docs/web-hosting/remote-access/ftp) or [SSH](/en/docs/web-hosting/remote-access/ssh),
...
2.  [Setup the site](/en/docs/web-hosting/sites/add-a-site) in **Web > Sites** with a preproduction address - e.g. the account address.
...
3.  Migrate the site addresses by modifying the DNS records for these subdomains and add the addresses in **Web > Sites**.
    ...
    - You can couple it using a [domain transfer](/en/docs/domains/transfer-a-domain).

> [!TIP]
> For the commands you can follow the [Move a site](/en/docs/web-hosting/sites/move-a-site) documentation instructions.
