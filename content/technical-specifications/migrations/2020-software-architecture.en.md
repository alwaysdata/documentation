...
In parallel to the Buster migration, we provide database migrations. You can [test](/technical-specifications/migrations/perform-migration) them via the **Test** button. All your databases and database users are copied to a temporary server, running the new versions.
...
For users of VPS and dedicated servers, the database migration test is associated to the Buster migration. The **Test** button will only **copy** the databases to the test server. Regarding HTTP deployment refer to the [Migration preparation](/technical-specifications/migrations/2020-software-architecture#migration-preparation) section.
