...
- **Technical at server level**: available on [Private Cloud](/admin-billing/billing/private-cloud-prices), your network administrator can manage the rules for the firewalls, e-mail send queues and many other aspects.
...
- **Resources**: permission for accounts on [Private Cloud](/admin-billing/billing/private-cloud-prices) allows to manage [probes](/web-hosting/sites/use-probes) (*Web > Probes* menu) and [resources](/technical-specifications/system-resources-alerts-and-limitations) (*Advanced > Resources* menu),
- **[Statistics](/web-hosting/analytics)**: analyze the visits to your sites (*Web > Analytics* menu),
- **[Sites](/web-hosting/sites)**: configure the websites and the Apache environment (*Web > Sites*, *Web > Configuration* menus),
...
- **[Databases](/web-hosting/databases)** (*Databases* menu),
- **[FTP](/web-hosting/remote-access/ftp)** (*Remote access > FTP* menu),
- **[SSH](/web-hosting/remote-access/ssh)** (*Remote access > SSH* menu),
- **[WebDAV](/web-hosting/remote-access/webdav)** (*Remote access > WebDAV* menu),
- **[Environment](/web-hosting/languages)**: configure the programming languages (*Environment* menu),
- **Processes**: processes currently executed that may be analyzed or killed (*Advanced > Processes* menu),
- **[IP addresses](/web-hosting/dedicated-ip-addresses)**: rent dedicated IPs for HTTP or SMTP (*Advanced > IP addresses* menu),
- **[SSL certificates](/web-hosting/sites/ssl-tls)** (*Advanced > SSL certificates* menu),
- **[Migration](/technical-specifications/migrations)** (*Advanced > Migrations* menu),
- **[Scheduled tasks](/web-hosting/tasks)** (*Advanced > Scheduled tasks* menu),
- **[Backups](/web-hosting/backups)** (*Advanced > Backup recovery* menu),
- **[Services](/web-hosting/services)** (*Advanced > Services* menu).
...
- **[SSH users](/web-hosting/remote-access/ssh/install-globally-ssh-keys)**: install SSH keys for simplified access to the various accounts (*SSH keys* menu),
- **[Firewall rules](/technical-specifications/configure-firewall)**: create firewall rules and refer to automatic IP banning by the server (*Firewall* menu),
...
- **SSL configuration**: choose the SSL certificate to return to the server (`*.alwaysdata.net` by default) and the server [TLS configuration](/web-hosting/sites/ssl-tls/configure-tls) (*SSL* menu),
- **HTTP configuration**: choose a website that will be the [default home page](/web-hosting/sites/misc#http-default-site) and the [log retention period](/web-hosting/remote-access/admin-directory#logs) (*HTTP* menu),
...
- **[Resources](/technical-specifications/system-resources-alerts-and-limitations)**: change the resource limits by account - RAM, CPU, disk space (*Resources* menu),
- **[Migration](/technical-specifications/migrations)** (*Migrations* menu) ;

## 2FA required

When the **2FA required** box is checked, the user in question must log in [with 2 factors](/admin-billing/profile/two-factor-authentication) to access the menus to which they have been given access.
