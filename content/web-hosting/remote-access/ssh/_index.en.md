...
**Remote SSH access is disabled by default.** To enable it, modify your user and check the password connection. It is then possible to set up a connection with [SSH keys](/web-hosting/remote-access/ssh/use-keys) and disable the password connection.

- [API - SSH](https://api.alwaysdata.com/v1/ssh/doc/)
- [Create an SSH user](/web-hosting/remote-access/ssh/create-a-ssh-user)
- [Frequent issues](/web-hosting/remote-access/ssh/troubleshooting)

* Users of [Private Cloud](/admin-billing/billing/private-cloud-prices):
	- [SSH 2-factor authentification](/web-hosting/remote-access/ssh/ssh-two-factor-authentication)
	- [Global SSH keys](/web-hosting/remote-access/ssh/install-globally-ssh-keys)
...
{{% notice info %}}
This interface is not compatible with the [Private Cloud](/admin-billing/billing/private-cloud-prices).
{{% /notice %}}
...
SSH users are not `chrooted`. Even if the user's root directory is not the root of the account, they will still be able to access the entire account. To use `chroot`, refer to [SFTP](/web-hosting/remote-access/sftp) or [FTP](/web-hosting/remote-access/ftp).
