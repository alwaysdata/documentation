...
The process analysis produces a file, available from the root of your account (called `analysis_[date]-[time].log` and that can be viewed in [SSH](/web-hosting/remote-access/ssh) or [FTP](/web-hosting/remote-access/ftp)) describing the operations performed by the process during a set period of time.
