...
The following commands, presented for a site that uses PHP, are run via [SSH](/web-hosting/remote-access/ssh) in your site's directory.
...
{{% notice tip %}}
If the infection is a **recent** one and the search for the infection a complex one, then it is possible to work from a [backup](/web-hosting/backups).
{{% /notice %}}
...
- Implement the [web application firewall](/web-hosting/sites/waf),
