+++
url = "/fr/hebergement-web/sites/ssl-tls/certificats-lets-encrypt/"
...
Pour éviter les doublons, alwaysdata autorise la génération de certificats Let's Encrypt seulement pour des [certificats wildcard](/web-hosting/sites/ssl-tls/lets-encrypt#certificats-wildcard).
...
Lorsqu'un domaine utilise nos [serveurs DNS](/technical-specifications/login-details), il est possible de générer un [certificat wildcard Let's Encrypt](https://en.wikipedia.org/wiki/Wildcard_certificate) - de la forme `*.example.org` - dans **Avancé > Certificats SSL > Ajouter un certificat SSL > Créer un certificat wildcard Let's Encrypt**. Ce certificat sera automatiquement renouvelé par le système.
